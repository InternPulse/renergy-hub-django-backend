    ...
    def get(self, request):
        try:
            ...
            products = (
                ...
                .annotate(total_sold=Sum('salesrecord__quantity'))
...
class ProductEngagementView(APIView):
    """
    View to handle product engagements (view, click) and retrieve analytics.
    """
    ...
    def post(self, request):
        ...
        if not product_id or not engagement_type:
            return Response(
                {'error': 'Product ID and Engagement Type are required.'},
...
class ProfitMarginView(APIView):
    ...
    def get(self, request):
        try:
            products = Product.objects.all()  # Ensure this line exists
            paginator = CustomPagination()
            paginated_products = paginator.paginate_queryset(products, request)
